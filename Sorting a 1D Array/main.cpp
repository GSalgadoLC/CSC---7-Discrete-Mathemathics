...
int  read(char array[]){   //Function to ready array size
    int sizeDet = 0;
    cin>>array;

    return strlen(array);  //Returns the size of the array 
...
void sort(char array[], int sizeIn){
        ...
        for ( int i = 0; i < sizeIn-1; i++){            //Cycles throught the rows
            
                for ( int j = 0; j < sizeIn-1 ; j++){   //Cycles through the collumns
                        
                        if (array[j]>array[j+1]){  //Compares the values of the current value  
                            char temp = array[j];   //If greater then a swap is performed
                            array[j] = array[j+1];
                            array[j+1] = temp;
                        }
...
void print(const char array[], int sizeIn){ //Displays out the sorted array
        cout<<array<<endl;
}
